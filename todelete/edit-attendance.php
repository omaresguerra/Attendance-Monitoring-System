...
<style type="text/css">
  ...
  .input { width: 100%; padding: 12px 15px; margin: 5px 0 5px; display: inline-block; border: 1px solid #b6b6b6; box-sizing: border-box; font-family:Verdana,sans-serif; font-size:15px; border-radius: 4px; color: #a0a0a0;}
   ...
   <div class="modal">
        <div class="modal-content">
          ...
          <?php
              ...
              echo "<form class=container action=update-attendance.php?acctype=".$acctype." method=POST>";
               
               while ($row = mysqli_fetch_array($result)) {
                     echo "<label><b>AttendID</b></label>";
                     echo "<div class=hide></div>";
                     echo "<input class=input type='text' name='id' value='".$row['AttendID']."' disable=true>";
                     echo "<label><b>Student</b></label>";

                     $qry = "SELECT * FROM student";
                     $result1 = mysqli_query($dbcon, $qry);
                     echo "<select class=input name='cboxStudent'onmousedown='if(this.options.length>2){this.size=2;}'   onchange='this.size=0;' onblur='this.size=0;'>";
                       while ($row1 = mysqli_fetch_array($result1)) {
                          echo "<option value='".$row1['StudID']."'>".$row1['StudName']."</option>";
                       }
                     echo "</select>";
                     ...
                     echo "<select class=input name='cboxEvent' onmousedown='if(this.options.length>2){this.size=2;}'   onchange='this.size=0;' onblur='this.size=0;'>";
                       while ($row2 = mysqli_fetch_array($result2)) {
                          echo "<option value='".$row2['EventID']."'>".$row2['EventName']."</option>";
                       }
                     echo "</select>";
                     echo "<label><b>Time</b></label>";
                     echo "<input class=input type='text' name='time' value='".$row['AttendTime']."'>";
                     echo "<div class=container-footer>";
                     echo "<button type=submit class=btn>Update</button></div></form>";
               } 
              ?>
              <?php
               $acctype = $_GET['acctype'];
